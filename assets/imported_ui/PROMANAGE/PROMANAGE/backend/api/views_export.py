...
from .models import Project, Work, Activity
...
def export_project_excel(request, project_id):
    ...
    cell = ws['A1']
    cell.value = f"LAPORAN PROYEK: {project.name.upper()}"
    cell.font = Font(bold=True, size=16, color="1E3A8A")
    cell.alignment = Alignment(horizontal="center", vertical="center")
    
    ws.merge_cells('A2:F2')
    ws['A2'].value = f"Dihasilkan pada {datetime.datetime.now().strftime('%d %B %Y, %H:%M')} oleh {request.user.name}"
    ws['A2'].alignment = Alignment(horizontal="center", vertical="center")
    
    # Project Details Section
    ws.append([])
    details = [
        ("Nama Proyek", project.name, "Status", project.status),
        ("Deskripsi", project.description, "Progres", f"{project.progress}%"),
        ("Lokasi", project.location, "Supervisor", project.supervisor),
        ("Tanggal Mulai", project.start_date.strftime('%Y-%m-%d'), "Pelaksana", project.executor),
        ("Tanggal Selesai", project.end_date.strftime('%Y-%m-%d'), "", "")
    ]
    ...
    def build_activity_table(title, activities_func, is_done):
        ...
        for work in project.works.all():
            ...
            for activity in acts:
                has_data = True
                row = [work.name, activity.name, activity.executor, activity.execution_time, "Selesai" if is_done else "Dalam Proses"]
                ws.append(row)
                for col_idx in range(1, 6):
    ...
    build_activity_table("AKTIVITAS SELESAI", lambda w: [a for a in w.activities.all() if a.done], True)
    build_activity_table("AKTIVITAS DALAM PROSES", lambda w: [a for a in w.activities.all() if not a.done], False)

    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = f'attachment; filename="Report_{project.name.replace(" ", "_")}.xlsx"'
    wb.save(response)
    return response
...
def export_project_pdf(request, project_id):
    ...
    try:
        ...
        response['Content-Disposition'] = f'attachment; filename="Report_{project.name.replace(" ", "_")}.pdf"'
        ...
        info_data = [
            [Paragraph("<b>Nama Proyek:</b>", normal_style), Paragraph(project.name, normal_style), Paragraph("<b>Status:</b>", normal_style), Paragraph(f"<font color='{get_status_color(project.status)}'><b>{project.status.upper()}</b></font>", normal_style)],
            [Paragraph("<b>Tanggal Mulai:</b>", normal_style), Paragraph(project.start_date.strftime('%d %b %Y'), normal_style), Paragraph("<b>Progres:</b>", normal_style), Paragraph(f"<b>{project.progress}%</b>", normal_style)],
            [Paragraph("<b>Tanggal Selesai:</b>", normal_style), Paragraph(project.end_date.strftime('%d %b %Y'), normal_style), Paragraph("<b>Supervisor:</b>", normal_style), Paragraph(project.supervisor, normal_style)],
            [Paragraph("<b>Lokasi:</b>", normal_style), Paragraph(project.location, normal_style), Paragraph("<b>Pelaksana:</b>", normal_style), Paragraph(project.executor, normal_style)],
        ]
        ...
        for idx, work in enumerate(works, 1):
            # Work Header Box
            work_header_data = [[
                Paragraph(f"<b>{idx}. {work.name.upper()}</b>", ParagraphStyle('WHT', parent=normal_style, textColor=colors.white)),
            ...
            for act in activities:
                status_text = "<font color='#059669'><b>Selesai</b></font>" if act.done else "<font color='#d97706'><b>Proses</b></font>"
                act_data.append([
                    Paragraph(act.name, small_style),
                    Paragraph(act.execution_time, small_style),
                    Paragraph(act.executor, small_style),
                    Paragraph(status_text, small_style)
                ])
                
                # Append Evaluation & Plan if exists
                if act.evaluation or act.additional_plan:
                    eval_plan_text = ""
                    if act.evaluation: eval_plan_text += f"<b>Evaluasi:</b> {act.evaluation}<br/>"
                    if act.additional_plan: eval_plan_text += f"<b>Rencana:</b> {act.additional_plan}"
        ...
        sig_data = [
            ...
            [Paragraph(f"<b><u>{request.user.name}</u></b><br/>Project Manager", normal_style), 
             Paragraph(f"<b><u>{project.supervisor}</u></b><br/>Project Supervisor", normal_style)]
        ]
        ...
        doc.build(elements, 
                  onFirstPage=lambda c, d: draw_header_footer(c, d, f"LAPORAN PROYEK - {project.name[:20]}", request.user.name),
                  onLaterPages=lambda c, d: draw_header_footer(c, d, f"LAPORAN PROYEK - {project.name[:20]}", request.user.name))
        return response
...
def export_all_excel(request):
    """Export all user projects to Professional Excel"""
    ...
    ws.merge_cells('A2:F2')
    ws['A2'].value = f"Dicetak oleh: {request.user.name} | Tanggal: {datetime.datetime.now().strftime('%d %B %Y')}"
    ws['A2'].alignment = Alignment(horizontal="center", vertical="center")
    ...
    for p in projects:
        ws.append([p.name, p.location, p.start_date.strftime('%Y-%m-%d'), p.end_date.strftime('%Y-%m-%d'), f"{p.progress}%", p.status])
        for col_idx in range(1, 7):
            apply_excel_data_style(ws.cell(row=ws.max_row, column=col_idx))
    ...
    for p in projects:
        ws.append([f"PROYEK: {p.name.upper()}"])
        ...
        for w in p.works.all():
            has_works = True
            acts = w.activities.all()
            if not acts:
                ws.append([w.name, "Tidak ada aktivitas", "-", "-", "-"])
                for col_idx in range(1, 6): apply_excel_data_style(ws.cell(row=ws.max_row, column=col_idx))
            for a in acts:
                ws.append([w.name, a.name, a.executor, a.execution_time, "Selesai" if a.done else "Proses"])
                for col_idx in range(1, 6): apply_excel_data_style(ws.cell(row=ws.max_row, column=col_idx))
...
def export_all_pdf(request):
    ...
    try:
        ...
        for p in projects:
            summary_data.append([
                Paragraph(f"<b>{p.name}</b>", normal_style),
                Paragraph(p.location, small_style),
                Paragraph(f"{p.start_date.strftime('%d/%m/%y')} - {p.end_date.strftime('%d/%m/%y')}", small_style),
                Paragraph(f"<b>{p.progress}%</b>", normal_style),
        ...
        for p in projects:
            # Project Heading
            p_head_data = [[Paragraph(f"<b>PROYEK: {p.name.upper()}</b>", ParagraphStyle('PH', parent=normal_style, textColor=colors.HexColor('#1e3a8a'), fontSize=12))]]
            ...
            for work in works:
                elements.append(Paragraph(f"<b>Pekerjaan: {work.name}</b>", normal_style))
                elements.append(Spacer(1, 5))
                
                act_data = [["Aktivitas", "Waktu", "Pelaksana", "Status"]]
                for act in work.activities.all():
                    status_str = "<font color='#059669'>Selesai</font>" if act.done else "<font color='#d97706'>Proses</font>"
                    act_data.append([
                        Paragraph(act.name, small_style),
                        Paragraph(act.execution_time, small_style),
                        Paragraph(act.executor, small_style),
                    ...
                    if act.evaluation or act.additional_plan:
                        eval_txt = ""
                        if act.evaluation: eval_txt += f"<b>Eval:</b> {act.evaluation}<br/>"
                        if act.additional_plan: eval_txt += f"<b>Renc:</b> {act.additional_plan}"
        ...
        doc.build(elements, 
                  onFirstPage=lambda c, d: draw_header_footer(c, d, "LAPORAN PORTOFOLIO GLOBAL", request.user.name),
                  onLaterPages=lambda c, d: draw_header_footer(c, d, "LAPORAN PORTOFOLIO GLOBAL", request.user.name))
        return response
    except Exception as e:
