...
import { Plus, FolderKanban, Calendar, MapPin, Users, Trash2, Search, ArrowRight, ArrowLeft, Briefcase, CheckCircle2, X, Download, FileText, FileSpreadsheet } from 'lucide-react';
...
export function ProjectManagement() {
  const { projects, loading, addProject, addWork, addActivity, deleteProject, renameProject } = useProjects();
  ...
  const DRAFT_KEY = 'promanage_project_wizard_draft';
  ...
  useEffect(() => {
    ...
    if (savedDraft) {
      try {
        ...
        setTempWorks(draft.tempWorks);
        setTempActivities(draft.tempActivities);
        setCurrentStep(draft.currentStep);
        setShowCreateModal(true);
        toast.info('Melanjutkan draf proyek yang belum selesai.');
      } catch (error) {
  ...
  }, [projectData, tempWorks, tempActivities, currentStep, showCreateModal]);
  ...
  const handleFinalSubmit = async () => {
    ...
    for (let i = 0; i < tempWorks.length; i++) {
      const work = tempWorks[i];
      const workId = await addWork({ ...work, projectId: newProjectId });
      const activities = tempActivities[i] || [];
      for (const activity of activities) {
        await addActivity(workId, activity);
      }
    }

    // Post to external API as requested
    try {
      const externalApiData = {
        "nama_proyek": projectData.name,
        "deskripsi_proyek": projectData.description,
        "pelaksana_proyek": projectData.executor,
        "supervisor_proyek": projectData.supervisor
      };
      ...
      const response = await fetch('http://192.168.2.225:8000/api/projects/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('External API returned error:', response.status, errData);
      }
    } catch (error) {
      console.error('External API Connection Error (Check CORS):', error);
    }
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
            <div>
              <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xl font-black text-gray-900 tracking-tight">Daftar Pekerjaan</h2>
                  <span id="work-count" className="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-xs font-bold">0 Pekerjaan</span>
              </div>
              <div id="activity-summary" className="mb-6">
                  <canvas id="activityChart" width="400" height="200"></canvas>
              </div>
              <div id="works-container" className="space-y-6">
                  {/*Work cards will be rendered here*/}
              </div><p className="text-gray-600 text-sm sm:text-base">Kelola dan pantau semua proyek Anda</p>
            </div>

            <div className="flex flex-wrap items-center gap-2 sm:gap-4 w-full sm:w-auto">
              {/* Global Export Section */}
              <div className="flex items-center gap-2 bg-white p-1 px-2 rounded-lg border border-gray-200 shadow-sm">
                <span className="text-[10px] font-bold text-gray-400 uppercase hidden xs:block">Global Export:</span>
                <button
                  onClick={() => api.export.downloadAllPdf()}
                  className="p-2 text-red-700 hover:bg-red-50 rounded-md transition-colors"
                  title="Ekspor Semua PDF"
                >
                  <FileText className="w-4 h-4" />
                </button>
                <button
                  onClick={() => api.export.downloadAllExcel()}
                  className="p-2 text-green-700 hover:bg-green-50 rounded-md transition-colors"
                  title="Ekspor Semua Excel"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className="bg-white rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-lg transition-shadow border border-gray-100 group relative flex flex-col h-full"
              >
                <Link to={`/project/${project.id}`} className="flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-bold text-red-800">{project.progress || 0}%</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-50">
                      <span className="text-[10px] font-bold text-gray-400 uppercase">Ekspor:</span>
                      <button 
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); api.export.downloadPdf(project.id, project.name); }}
                        className="flex items-center gap-1 text-[10px] bg-red-50 hover:bg-red-100 text-red-800 px-2 py-1 rounded font-bold transition-colors"
                      >
                        <FileText className="w-3 h-3" />
                        PDF
                      </button>
                      <button 
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); api.export.downloadExcel(project.id, project.name); }}
                        className="flex items-center gap-1 text-[10px] bg-green-50 hover:bg-green-100 text-green-800 px-2 py-1 rounded font-bold transition-colors"
                      >
                ...
                {steps.map((step, i) => (
                  <div key={step.num} className="flex items-center flex-1">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${currentStep >= step.num ? 'bg-red-800 text-white' : 'bg-gray-200 text-gray-500'
                        }`}>
                        {currentStep > step.num ? <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5" /> : step.num}
                      </div>
                      <span className={`font-medium text-xs sm:text-sm hidden xs:inline sm:inline transition-colors ${currentStep >= step.num ? 'text-red-800' : 'text-gray-400'
                        }`}>{step.label}</span>
                    </div>
                    {i < steps.length - 1 && (
